...
def visualize_whales_bybit_orderbook(token: str):
    ...
    whales_bids = whales_bids[whales_bids['filled_amount']
                              <= 0.8*whales_bids['total_amount']]
    whales_asks = whales_asks[whales_asks['filled_amount']
                              <= 0.8*whales_asks['total_amount']]
    ...
    st.write(pd.concat([
        agg_bids_df.rename(
            columns={c: f'{c}_bid' for c in agg_asks_df.columns}),
        agg_asks_df.rename(columns={c: f'{c}_ask' for c in agg_asks_df.columns})],
        axis=1)[['exchange_bid', 'collateral_quoted_in_bid', 'order_amount_bid', 'quantity_bid', 'price_bid',
                 'price_ask', 'quantity_ask', 'order_amount_ask', 'collateral_quoted_in_ask', 'exchange_ask']])
...
#  & set(gateio_tokens['currency'].unique().tolist())
    ...
    visualize_whales_bybit_orderbook(selected_token)
